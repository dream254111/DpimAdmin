...
String.prototype.indexOfIgnoreCase = function (c) {
...
let $_isNullOrEmpty = function (x) {
    return String.isNullOrEmpty(x);
}

let $_round = function (x, d) {
    d = $_isNullOrEmpty(d) ? 0 : d;
    return $_isNullOrEmpty(x) || isNaN(x) ? null : Math.round2(x, d);
}

let $_int = function (x) {
    return $_isNullOrEmpty(x) || isNaN(x) ? null : parseInt(x);
}

let $_float = function (x) {
    return $_isNullOrEmpty(x) || isNaN(x) ? null : $_round(parseFloat(x), 8);
}